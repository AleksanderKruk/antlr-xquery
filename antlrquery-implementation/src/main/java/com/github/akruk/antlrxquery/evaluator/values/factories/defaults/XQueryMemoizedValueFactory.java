...
public class XQueryMemoizedValueFactory implements XQueryValueFactory {
    ...
    @Override
    public XQueryValue node(String grammar, ParseTree v) {
        // TODO:
        String name = ParseTree.class.getName();
        return createdNodes.computeIfAbsent(
                v, _ -> XQueryValue.node(v, typeFactory.element(grammar, Set.of(new NamespaceResolver.QualifiedName(grammar, name)))));
    }

    @Override
    public XQueryValue number(BigDecimal d) {
        // TODO:
        return createdNumbers.computeIfAbsent(d, _ -> XQueryValue.number(d, typeFactory.number(NumericRange.FULL)));
    }
    @Override
    public XQueryValue number(int integer) {
        return createdIntegers.computeIfAbsent(integer, _ -> XQueryValue.number(integer, typeFactory.number(NumericRange.FULL)));
    }
    ...
    public XQueryValue sequence(List<XQueryValue> v) {
        ...
        return createdSequences.computeIfAbsent(
            v, _ -> XQueryValue.sequence(v, typeFactory.zeroOrMore(typeFactory.itemAnyItem())));
    }

    @Override
    public XQueryValue string(String s) {
        return createdStrings.computeIfAbsent(s, _ -> XQueryValue.string(s, typeFactory.string()));
    }
