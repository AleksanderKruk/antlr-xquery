...
public class ProcessingArrayFunctionsTest extends SemanticTestsBase {
    ...
    public void membersArray_valid() {
        assertType(
                ...
                typeFactory.zeroOrMore(typeFactory.itemRecord(
                        new LinkedHashMap<>(Map.of("value", new RecordField("value", new TypeOrReference.Type(typeFactory.anyItem()), true)))))
        )
        ;
    }
    ...
    // array:subarray($array as array(*), $start as xs:integer, $cardinality as
    // xs:integer? := ()) as array(*)
    @Test
