...
public final class MemoizedCardinalityFactory implements CardinalityFactory {
    ...
    private final ConcurrentMap<BigDecimal, FiniteBound> boundCache =
    ...
    public MemoizedCardinalityFactory() {
        ...
        EXACTLY_ONE = intern(
                event(bound(BigDecimal.ONE), Type.START),
                event(bound(BigDecimal.ONE), Type.END)
        );


        ZERO_OR_ONE = intern(
                event(bound(BigDecimal.ZERO), Type.START),
                event(bound(BigDecimal.ONE), Type.END)
        );
    ...
    private FiniteBound bound(BigDecimal value) {
        BigDecimal normalized = value.stripTrailingZeros();

        return boundCache.computeIfAbsent(
                normalized,
                FiniteBound::new
        );
    }
