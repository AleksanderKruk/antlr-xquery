...
public class SubstringMatchingFunctionsTest extends FunctionsSemanticTest {

    // fn:contains($value as xs:string?, $substring as xs:string?, $collation as xs:string? := default) as xs:boolean
    @Test public void contains_positional() {
        assertType("fn:contains('abracadabra','cad')",
            typeFactory.one(typeFactory.itemBoolean()));
    }
    @Test public void contains_namedArgs() {
        assertType("fn:contains(substring := 'bra', value := 'abracadabra')",
            typeFactory.one(typeFactory.itemBoolean()));
    }
    @Test public void contains_withCollation() {
        assertType("fn:contains('a','A','uci')",
            typeFactory.one(typeFactory.itemBoolean()));
    }
    @Test public void contains_defaultCollation() {
        assertType("fn:contains('','')",
            typeFactory.one(typeFactory.itemBoolean()));
    }
    @Test public void contains_tooFewArguments() {
        assertErrors("fn:contains('a')");
        assertErrors("fn:contains()");
    }
    @Test public void contains_tooManyArguments() {
        assertErrors("fn:contains('a','b','c','d')");
    }
    @Test public void contains_wrongTypes() {
        assertErrors("fn:contains(1,'x')");
        assertErrors("fn:contains('x',1)");
        assertErrors("fn:contains('x','y',1)");
    }

    // fn:starts-with($value as xs:string?, $substring as xs:string?, $collation as xs:string? := default) as xs:boolean
    @Test public void startsWith_positional() {
        assertType("fn:starts-with('hello','he')",
            typeFactory.one(typeFactory.itemBoolean()));
    }
    @Test public void startsWith_namedArgs() {
        assertType("fn:starts-with(substring := 'lo', value := 'hello')",
            typeFactory.one(typeFactory.itemBoolean()));
    }
    @Test public void startsWith_withCollation() {
        assertType("fn:starts-with('X','x','uci')",
            typeFactory.one(typeFactory.itemBoolean()));
    }
    @Test public void startsWith_missingArguments() {
        assertErrors("fn:starts-with('x')");
        assertErrors("fn:starts-with()");
    }
    @Test public void startsWith_wrongTypes() {
        assertErrors("fn:starts-with(1,'h')");
        assertErrors("fn:starts-with('h',1)");
        assertErrors("fn:starts-with('h','h',true())");
    }

    // fn:ends-with($value as xs:string?, $substring as xs:string?, $collation as xs:string? := default) as xs:boolean
    @Test public void endsWith_positional() {
        assertType("fn:ends-with('testing','ing')",
            typeFactory.one(typeFactory.itemBoolean()));
    }
    @Test public void endsWith_namedArgs() {
        assertType("fn:ends-with(value := 'abc', substring := 'bc')",
            typeFactory.one(typeFactory.itemBoolean()));
    }
    @Test public void endsWith_withCollation() {
        assertType("fn:ends-with('A','a','uci')",
            typeFactory.one(typeFactory.itemBoolean()));
    }
    @Test public void endsWith_arityErrors() {
        assertErrors("fn:ends-with('a')");
        assertErrors("fn:ends-with()");
        assertErrors("fn:ends-with('a','b','c')");
    }
    @Test public void endsWith_wrongTypes() {
    ...
    @Test public void substringBefore_wrongTypes() {
    ...
    @Test public void substringAfter_wrongTypes() {
