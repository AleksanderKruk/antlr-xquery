...
public class ComparisonFunctionsTest extends FunctionsSemanticTest {
    ...
    @Test public void atomicEqual_wrongTypes() {
        ...
        assertErrors("fn:atomic-equal(true(), <b/>)");
    }
    @Test public void atomicEqual_arityErrors() {
    ...
    @Test public void deepEqual_badOptionType() {
        assertErrors("fn:deep-equal((1),(1), 123)");
    }

    // fn:compare($v1 as xs:anyAtomicType?,$v2 as xs:anyAtomicType?, $collation as xs:string? := default) as xs:integer?
    @Test public void compare_noArgs() {
        assertType("fn:compare()",
            typeFactory.zeroOrOne(typeFactory.itemNumber()));
    }
    @Test public void compare_minimal() {
        assertType("fn:compare('a','b')",
            typeFactory.zeroOrOne(typeFactory.itemNumber()));
    }
    @Test public void compare_withCollation() {
        assertType("fn:compare('a','b','uci')",
            typeFactory.zeroOrOne(typeFactory.itemNumber()));
    }
    @Test public void compare_namedCollation() {
        assertType("fn:compare(v2 := 'b', v1 := 'a', collation := 'uc')",
            typeFactory.zeroOrOne(typeFactory.itemNumber()));
    }
    @Test public void compare_wrongTypes() {
        assertErrors("fn:compare(<a/>, 'b')");
        assertErrors("fn:compare('a', <b/>)");
        assertErrors("fn:compare('a','b', 1)");
    }

    // fn:distinct-values($values as xs:anyAtomicType*, $collation as xs:string? := default) as xs:anyAtomicType*
    @Test public void distinctValues_empty() {
        assertType("fn:distinct-values()",
            typeFactory.zeroOrMore(typeFactory.itemAnyItem()));
    }
    @Test public void distinctValues_withValues() {
        assertType("fn:distinct-values('a','b','a')",
            typeFactory.zeroOrMore(typeFactory.itemAnyItem()));
    }
    @Test public void distinctValues_namedCollation() {
        assertType("fn:distinct-values(1,2,1, collation := 'uc')",
            typeFactory.zeroOrMore(typeFactory.itemAnyItem()));
    }
    @Test public void distinctValues_wrongType() {
        assertErrors("fn:distinct-values(<x/>)");
    }

    // fn:duplicate-values($values as xs:anyAtomicType*, $collation as xs:string? := default) as xs:anyAtomicType*
    @Test public void duplicateValues_empty() {
        assertType("fn:duplicate-values()",
            typeFactory.zeroOrMore(typeFactory.itemAnyItem()));
    }
    @Test public void duplicateValues_withValues() {
        assertType("fn:duplicate-values('x','x','y')",
            typeFactory.zeroOrMore(typeFactory.itemAnyItem()));
    }
    @Test public void duplicateValues_wrongType() {
        assertErrors("fn:duplicate-values(<x/>)");
    }

    // fn:index-of($in as xs:anyAtomicType*, $target as xs:anyAtomicType, $collation as xs:string? := default) as xs:integer*
    @Test public void indexOf_minimal() {
        assertType("fn:index-of(1, 2, 'uc')",
            typeFactory.zeroOrMore(typeFactory.itemNumber()));
    }
    @Test public void indexOf_defaultCollation() {
        assertType("fn:index-of('a','a')",
            typeFactory.zeroOrMore(typeFactory.itemNumber()));
    }
    @Test public void indexOf_missingOrExtra() {
        assertErrors("fn:index-of(1)");
        assertErrors("fn:index-of()");
        assertErrors("fn:index-of(1,2,'x','y')");
    }
    @Test public void indexOf_wrongTypes() {
