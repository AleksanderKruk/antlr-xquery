...
public class ProcessingMapFunctionsTest extends FunctionsSemanticTest {
    ...
    @Test public void getMap_errors() {
        assertErrors("map:get()");
        assertErrors("map:get(map{}, <a/>)");
    }

    // map:items($map as map(*)) as item()*
    @Test public void itemsMap_valid() {
        assertType(
            "map:items(map{'a':1})",
            typeFactory.zeroOrMore(typeFactory.itemAnyItem())
        );
    }
    @Test public void itemsMap_errors() {
        assertErrors("map:items()");
        assertErrors("map:items(1)");
    }

    // map:keys($map as map(*)) as anyAtomicType*
    @Test public void keysMap_valid() {
        assertType(
            "map:keys(map{'a':1})",
            typeFactory.zeroOrMore(typeFactory.itemAnyItem())
        );
    }
    @Test public void keysMap_errors() {
        assertErrors("map:keys()");
    }

    // map:keys-where($map as map(*), $predicate as fn(item(),item()*) as xs:boolean?) as anyAtomicType*
    @Test public void keysWhere_valid() {
        assertType(
            "map:keys-where(map{'a':1}, function($k,$v){ true() })",
            typeFactory.zeroOrMore(typeFactory.itemAnyItem())
        );
    }
    @Test public void keysWhere_errors() {
        assertErrors("map:keys-where()");
        assertErrors("map:keys-where(map{}, 1)");
    }

    // map:merge($maps as map(*)*, $options as map(*)? := {}) as map(*)
    @Test public void mergeMaps_valid() {
        assertType(
