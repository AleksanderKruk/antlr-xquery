...
def detect_black_shapes(img, min_area=100):
    """
    Wykrywa czarne kształty na obrazie i zwraca listę konturów
    o powierzchni większej niż min_area.
    """
    # Konwersja do skali szarości
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

    # Progowanie dla czarnych kształtów
    # Niskie wartości (<50) będą traktowane jako czarne
    ...
    # Tworzenie obrazu do wizualizacji
    black_shapes_img = img.copy()
    cv2.drawContours(black_shapes_img, large_contours, -1, (0, 255, 255), 2)

    # Zwróć listę konturów i obraz z wizualizacją
    return large_contours, black_shapes_img
...
def detect_and_warp_red_ring(image_path):
    ...
    # Wykryj czarne kształty w oryginalnym obrazie
    black_contours_orig, black_shapes_img_orig = detect_black_shapes(img, min_area=100)

    # Wykryj czarne kształty w przetransformowanym obrazie
    black_contours_warped, black_shapes_img_warped = detect_black_shapes(warped, min_area=100)

    # Zwróć wyniki do porównania
    return {
        "perspective_warped": warped,
        "detected_contour": img_contours,
        "black_shapes_orig": black_shapes_img_orig,
        "black_contours_orig": black_contours_orig,
        "black_shapes_warped": black_shapes_img_warped,
        "black_contours_warped": black_contours_warped
    }
...
def main():
    ...
    if result:
        ...
        cv2.imwrite("black_shapes_orig.jpg", result["black_shapes_orig"])
        cv2.imwrite("black_shapes_warped.jpg", result["black_shapes_warped"])

        # Wyświetlenie informacji o znalezionych czarnych kształtach w oryginalnym obrazie
        print(f"Znaleziono {len(result['black_contours_orig'])} czarnych kształtów o powierzchni > 100 pikseli w oryginalnym obrazie:")
        for i, contour in enumerate(result['black_contours_orig']):
            area = cv2.contourArea(contour)
            x, y, w, h = cv2.boundingRect(contour)
            print(f"  Kształt {i+1}: Powierzchnia = {area:.2f} pikseli, Pozycja: x={x}, y={y}, szerokość={w}, wysokość={h}")

        # Wyświetlenie informacji o znalezionych czarnych kształtach w przetransformowanym obrazie
        print(f"\nZnaleziono {len(result['black_contours_warped'])} czarnych kształtów o powierzchni > 100 pikseli w przetransformowanym obrazie:")
        for i, contour in enumerate(result['black_contours_warped']):
            area = cv2.contourArea(contour)
            x, y, w, h = cv2.boundingRect(contour)
            print(f"  Kształt {i+1}: Powierzchnia = {area:.2f} pikseli, Pozycja: x={x}, y={y}, szerokość={w}, wysokość={h}")
        ...
        cv2.imshow("Black Shapes Original", result["black_shapes_orig"])
        cv2.imshow("Black Shapes Warped", result["black_shapes_warped"])
        cv2.waitKey(-1)
